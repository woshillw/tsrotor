#include "tsrotor_adrc_controller_node.h"
    ...
    , _odometry_got(false)
{
  ...
  _actuator_servo_position_pub = _nh.advertise<mav_msgs::Actuators>("/tsrotor/gazebo/command/servo_position", 1);

  _actuator_motor_speed_pub = _nh.advertise<mav_msgs::Actuators>("/tsrotor/gazebo/command/motor_speed", 1);
  ...
  _state_xpos_pub = _nh.advertise<std_msgs::Float64>("state/xpos", 1);
  _state_ypos_pub = _nh.advertise<std_msgs::Float64>("state/ypos", 1);
  _state_zpos_pub = _nh.advertise<std_msgs::Float64>("state/zpos", 1);
...
void OmniAdrcControllerNode::InitializeParams()
{
  ...
  Eigen::Matrix<double, 6, 12> angular_theta_to_fm_b;
  ...
  angular_theta_to_fm_b << 0, -kf / 2, 0, -kf, 0, -kf / 2, 0, kf / 2, 0, kf, 0, kf / 2,
  ...
  _allocationMatrixs = Pinv(angular_theta_to_fm_b);
...
void OmniAdrcControllerNode::OdometryCallback(const nav_msgs::Odometry &odometry)
{
  ...
  msg.data = _state_pos(0);
  _state_xpos_pub.publish(msg);
  msg.data = _state_pos(1);
  _state_ypos_pub.publish(msg);
  msg.data = _state_pos(2);
  _state_zpos_pub.publish(msg);
}
...
void OmniAdrcControllerNode::ControlAllocation()
{
  ...
  FM_b << F_b, _M_b;
    ...
    VirtualControl = _allocationMatrixs * FM_b;
    double motor[6];//
    double alpha[6];//
    ...
    actuator_msg->angular_velocities.push_back(motor[0]);
    actuator_msg->angular_velocities.push_back(motor[1]);
    actuator_msg->angular_velocities.push_back(motor[2]);
    actuator_msg->angular_velocities.push_back(motor[3]);
    actuator_msg->angular_velocities.push_back(motor[4]);
    actuator_msg->angular_velocities.push_back(motor[5]);
  
    _actuator_motor_speed_pub.publish(actuator_msg);

    actuator_msg->angular_velocities.clear();
    actuator_msg->angular_velocities.push_back(alpha[0]);
    actuator_msg->angular_velocities.push_back(alpha[1]);
    actuator_msg->angular_velocities.push_back(alpha[2]);
    actuator_msg->angular_velocities.push_back(alpha[3]);
    actuator_msg->angular_velocities.push_back(alpha[4]);
    actuator_msg->angular_velocities.push_back(alpha[5]);
